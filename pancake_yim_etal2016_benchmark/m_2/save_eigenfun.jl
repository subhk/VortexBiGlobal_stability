...
# ╔═╡ 805dbdce-b2dc-11ee-26ae-d55a0e628ebf
begin 
	using LazyGrids
	using BlockArrays
	using Printf
...
# ╔═╡ b9822582-fd70-4168-9766-45fc98313128
begin 
...
# ╔═╡ eb2728ce-8ecd-413f-ba7a-507d2926e884
begin 
...
# ╔═╡ a07b70c8-5c56-4972-ba3f-f6a513b5d132
begin 
	which_file = 2
	no_eqs = 5
	Nr = 160
    ...
    eigenvec = readdlm("real_eigenvec_" * string(which_file) * ".dat", 
					'\t', Float64, '\n')
    ...
    r  = collect(range(0.0,  stop=10.0, length=Nr))
    z  = collect(range(-5.0, stop=5.0,  length=Nz))
    # Dʳ = ddz(r); Dʳʳ = ddz2(r);
    # Dᶻ = ddz(z); Dᶻᶻ = ddz2(z);
    ...
    fig = Figure(fontsize=40, size = (1700, 580)) #, font="Times")

    ax1 = Axis(fig[1, 1], 
		xlabel=L"$r/R$", xlabelsize=40, ylabel=L"$z/H$", ylabelsize=40)

    interp_  = Interp2D_eigenFun(r, z, uᵣ, r_interp, z_interp)
    max_val = maximum(abs.(interp_))
    levels = range(-0.7max_val, 0.7max_val, length=12)
    ...
    # contour!(r_interp, z_interp, U₀, levels=levels, 
	...
	ax1.yticks=([-1, -0.5, 0, 0.5, 1], ["-1", "-0.5", "0", "0.5", "1"])
    ...
    # contour!(r_interp, z_interp, B₀, levels=levels, 
	# 			linestyle=:dash, color=:black, linewidth=2) 
	...
	Label(fig[1, 1, TopLeft()], "(a)", 
		fontsize=40, padding = (-2, 0, 20, 0), halign = :right)
	Label(fig[1, 3, TopLeft()], "(b)", 
		fontsize=40, padding = (-2, 0, 20, 0), halign = :right)


    ax1.title = L"\mathfrak{R}(\hat{u}_r)"
    ax2.title = L"\mathfrak{R}(\hat{u}_\theta)"

    fig
	save("benchmark_" * string(which_file) * ".png", fig, px_per_unit=4)
end
...
manifest_format = "2.0"
project_hash = "4a7a976026d05269c4a86cf6a148efb7f84a82ab"
...
[[deps.JacobiDavidson]]
deps = ["IterativeSolvers", "LinearAlgebra", "LinearMaps", "ProgressMeter", "Random", "Test"]
git-tree-sha1 = "665b30149209f4351d23a2261239426f21e4deba"
uuid = "11c68b98-9c9b-11e8-267b-bbb95576cead"
version = "0.1.1"
...
git-tree-sha1 = "53013d7f64ce6d78c1529f43ebfeccc2070fa764"
