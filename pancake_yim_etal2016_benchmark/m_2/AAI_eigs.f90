...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  do kk=2, 2
    ...
    if (kk < 10) then
        write (xx1, '(I0.0)') kk
        open(10, file = 'Ro_20/systemA_'//trim(xx1)//'.mtx', status='old')
    else
        write (xx2, '(I2.0)') kk
        open(10, file = 'Ro_20/systemA_'//trim(xx2)//'.mtx', status='old')
    endif
    print *, kk
  ...
  ! !!! B matrix 
    if (kk < 10) then
        open(10, file = 'Ro_20/systemB_'//trim(xx1)//'.mtx', status='old')
      else
        open(10, file = 'Ro_20/systemB_'//trim(xx2)//'.mtx', status='old')
    endif
    read(10,*)
    read(10,*) n,n,nnz
    allocate(ic(nnz))
  ...
  !!!!!!!!!!!!!  FEAST
    ...
    fpm(42) = 0     ! Linear solver: single-precision (1), double-precision (0) 
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (info==0) then
      ...
      if (kk < 10) then
        open(10, file = 'Ro_20/real_eigenvec_'//trim(xx1)//'.dat', status='new')
        open(20, file = 'Ro_20/imag_eigenvec_'//trim(xx1)//'.dat', status='new')
      else
        open(10, file = 'Ro_20/real_eigenvec_'//trim(xx2)//'.dat', status='new')
        open(20, file = 'Ro_20/imag_eigenvec_'//trim(xx2)//'.dat', status='new')
      endif
      do i=1,N
